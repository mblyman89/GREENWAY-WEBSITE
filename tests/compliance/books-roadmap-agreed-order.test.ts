...
describe("books-46: the roadmap's slice A claims are re-derived, not trusted", () => {
   ...
   *
   * 3. A `for` LOOP ROUND AN `it(` BREAKS THE SAME EQUIVALENCE, AND books-58
   *    PROVED THE `.each` REFUSAL DID NOT COVER IT.
   *
   *    `form-sheet-core.test.ts` walks `ALL_TAUGHT_FORM_IDS` in a plain `for`
   *    loop, declaring five `it(` blocks per form. Source says 29. The runner
   *    reports 64. It contains no `.each` whatever, so the refusal above waved
   *    it straight through, and the roadmap's "(64)" would have been compared
   *    against a static 29 and failed for the WRONG REASON - or, had the number
   *    been written as 29 to make the gate green, the document would have
   *    printed a figure that matches nothing a person ever sees.
   *
   *    That is the same defect as `it.each` wearing different syntax, and the
   *    fix is the same in spirit: refuse what cannot be counted statically
   *    rather than guess. So the source count is now taken ONLY from files with
   *    no loop around an `it(`, and any file that has one must print the
   *    RUNTIME count and be listed in RUNTIME_COUNTED below with its figure
   *    checked by running it. Rule 23 - fix the class, not the instance.
   */
  /**
   * Files whose tests are generated by a loop, with the count the RUNNER
   * reports - not the number of `it(` in the source.
   *
   * These figures cannot be derived by reading the file, so each one is a
   * measurement: run the file, read the total, record it here. Kept to the
   * files that genuinely need it, because every entry is a number a human has
  ...
  it("prints the true test count for every test file it names", () => {
     ...
     *
     * THE EIGHTH is form-sheet-core.test.ts (64), documented in books-58, and
     * it is the one that exposed a hole in this gate rather than merely
     * incrementing it. That file declares its `it(` blocks inside a `for` loop
     * over every taught form, so the source count is 29 and the runner reports
     * 64. It contains no `.each`, so the refusal above did not catch it. The
    ...
    for (const [, fileName, printedRaw] of rows) {
      ...
      /*
       * A file that declares `it(` inside a `for` loop cannot be counted by
       * grepping. Detected by indentation: an `it(` nested deeper than one
       * level of `describe` is inside something, and that something multiplies
       * it. Such files must print the RUNTIME count and appear below.
       */
      const hasLoopedTests = /^\s{4,}it\(/m.test(src);

      if (hasLoopedTests) {
        const expected = RUNTIME_COUNTED[fileName];
        expect(
          expected,
          `${fileName} declares it( inside a loop, so its source count is not its ` +
            `test count. Add it to RUNTIME_COUNTED with the figure the runner ` +
            `reports, or the roadmap's "(${printed})" is unverifiable.`,
        ).toBeDefined();
        expect(printed, `roadmap says ${fileName} has ${printed} tests`).toBe(expected);
        continue;
      }
