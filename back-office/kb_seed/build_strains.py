...
# Expansion pass 3 (deep-extraction) — verified, customer-facing strain facts
# combined from public datasets (WA I-502 state lab tests, multi-lab terpene
# assays, The Cannabis API, Kushy dataset). Generated by build_extracted.py.
# Appended only when the name (and its normalized form) is not already present,
# so the curated higher-confidence entries above always win. Sensory/botanical/
# market-factual ONLY — no medical or effect claims.
import re as _re


def _norm_key(_s):
    return _re.sub(r"[^a-z0-9]", "", _s.lower())


try:
    from strains_extracted import EXTRACTED_STRAINS
    _existing_names = {s[0].strip().lower() for s in STRAINS}
    _existing_norm = {_norm_key(s[0]) for s in STRAINS}
    for _row in EXTRACTED_STRAINS:
        _nm = _row[0].strip().lower()
        _nk = _norm_key(_row[0])
        if _nm not in _existing_names and _nk not in _existing_norm:
            STRAINS.append(_row)
            _existing_names.add(_nm)
            _existing_norm.add(_nk)
except ImportError:
    EXTRACTED_STRAINS = []
