...
| L-8 (CCTV retention / scale calibration / recurring compliance reminders) | S-18 | PR-I | Compliance calendar (`/admin/compliance/calendar`, nav-gated on `settings.manage`): five recurring WAC obligations tracked per period — LIQ-1295 excise return (monthly, prior month due the 20th), CCRS weekly upload (Sun–Sat week due following Saturday), CCTV 45-day retention spot-check + badge/visitor-log review (monthly, due month-end), scale calibration (annual, due Dec 31). PURE calendar-date engine (`lib/compliance/compliance-calendar-core.ts`, PlainDate math, no Date parsing) computes done/due/overdue per period; sign-offs stored in `site_settings` KV (`compliance_calendar_done`, no migration) with `{doneAt, byEmail}` and audited (`compliance_calendar.done`/`.undone`); missing data fails safe (everything shows due). Dashboard shows a red overdue nag linking to the calendar. 18 pure tests in `tests/compliance/compliance-calendar.test.ts`. |
