...
Sixty-one new tests cover the screen logic, on top of the tests for the join, the read-back, the store and the mentor. This report itself has forty-nine tests of its own, which re-derive every statutory quotation above from the stored text and recompute every number in it.
