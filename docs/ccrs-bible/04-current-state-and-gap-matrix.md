...
| N-04 header padding | **FLAGGED (default OFF)** — S-01 | templates pad header rows with commas `[TPL * R1-R3]` | `padHeaderRowsForTemplates` added to `ccrs-batch-core.ts`; **opt-in only**, NOT wired into `assembleCcrsFile`, goldens unchanged | Still blocked on **U-03**: prove in PREproduction whether CCRS requires padding. Both shapes are tested to pass `verifyCcrsFile`. If PREprod rejects the unpadded shape, wire it in and regenerate goldens | S-01 / U-03 |
| N-05 UTC file stamp | **DONE** — S-01 | "file name should be referenced in PST" `[FAQ L0075]` | `ccrsFileStamp` now uses `pacificParts()` (`America/Los_Angeles`), the existing shared helper — no second timezone implementation | Covered by `tests/compliance/ccrs-file-stamp.test.ts` (19 tests: DST spring-forward/fall-back, midnight, winter PST, 14-digit width). Goldens unchanged — the fixture instant is the same Pacific day | S-01 |
| N-06 gate self-test never run | **DONE** — S-01 | — | `__runCcrsSubmitGateTests` now imported and executed by `tests/compliance/pure-selftests.test.ts` (47 → 48 tests); the gate reports "all 10 tests passed" | Proven able to fail: mutation M7 (`submittable: true`) turns the suite red | S-01 |
