...
- **T-294 (Medical hero &amp; intro text now editable in the Medical page editor):** open the back-office **Medical page** editor (Website → Medical page). **Expect** a new **Hero &amp; intro** section at the top with three fields: the big **hero title**, the **hero subtitle**, and the **intro paragraph**. Change the **hero title**, **Publish**, and confirm the public **/medical** page’s headline updates. **Expect** these three fields to ALSO still appear in the **Site Content** editor for now (that harmless duplicate is removed in the next step) — editing in either place changes the same text. **Expect** the public /medical page to look and work EXACTLY as before until you edit and publish (byte-identical), and the purchase-limit table + legal fine print to stay fixed. **Red flag:** the Hero &amp; intro section is missing; a published change doesn’t appear on /medical; the two editors disagree; or any editor/page error.
- **T-295 (Medical &amp; legal page copy removed from the Site Content junk drawer (each now has ONE home)):** open the back-office **Site Content** editor (Website → Site Content). **Expect** the medical page copy (the /medical hero title, subtitle and intro paragraph) and ALL the legal-page copy (the Privacy, Terms and Consumer-Health-Data page titles plus their long policy bodies) to NO LONGER appear here. **Expect** the medical copy to now live ONLY in the **Medical page** editor (Website → Medical page, the Hero &amp; intro section added last step), and the legal copy to live ONLY in the **Legal &amp; Policies** editor. **Expect** the public **/medical**, **/privacy**, **/terms** and **/consumer-health-data** pages to look and read EXACTLY as before (byte-identical) until you edit and publish in those dedicated editors. **Red flag:** medical or legal blocks still show in Site Content; a public page changed on its own; editing in the dedicated editor doesn’t update the public page; or any editor error.
- **T-296 (Header &amp; Footer links removed from the Site Content junk drawer (each now has ONE home)):** open the back-office **Site Content** editor (Website → Site Content). **Expect** the header/footer items to NO LONGER appear here — the five **Follow Greenway** social links (Facebook, Instagram, Google, Yelp, Leafly), the two **app-download** links (Apple, Google Play), the **“not connected yet”** footer message, and the two **top-bar** controls (the store-hours text size and the phone-button text). **Expect** every one of them to now live ONLY in the **Header &amp; Footer** editor (Website → Header &amp; Footer), where they have always been editable. Change a social link there, **Publish**, and confirm the public site footer updates. **Expect** the public footer and top bar to look and work EXACTLY as before (byte-identical) until you edit and publish. **Red flag:** any header/footer item still shows in Site Content; a published change doesn’t reach the footer; or any editor error.
- **T-297 (Blog page wording gets its own editor (Blog & Newsletter -> Blog wording)):** open the new **Blog wording** editor (Blog &amp; Newsletter → Blog wording, or the ’Edit the blog page’s wording’ quick-link on the Blog screen). **Expect** to see the seven blog page-chrome fields grouped into **Blog landing page** (hero eyebrow, the three heading words ’Stories / Culture / Newsletters’, and the intro line), **Post cards** (the ’Read article’ button label), and **Single article page** (the ’Back to blog’ link). Edit a field, **Save draft**, then **Publish**, and confirm the public **/blog** page updates. **Expect** the public blog to look EXACTLY as before (byte-identical) until you edit and publish, and **Expect** the blog POSTS and the card design to be unaffected (this editor only changes wording). **Expect** these fields to ALSO still appear in Site Content for now (they move out in the next slice). **Red flag:** a published change doesn’t reach /blog; the editor shows a post or a card-design control; or any editor error.
- **T-298 (Blog wording leaves Site Content, moves to the Website menu, and the old Menu page editor is retired):** open **Site Content** and **Expect** the seven blog wording fields (blog.hero.*, the ’Read article’ label, the ’Back to blog’ link) to be GONE from the list — the Site Content ’junk drawer’ is now empty of blog copy. Confirm those same fields still live in the dedicated **Blog wording** editor, which now sits in the **Website** dropdown (moved out of Marketing &amp; Advertising) alongside the other page editors. Then open the **Website** dropdown and **Expect** the old **Menu** page editor to be GONE; visiting **/admin/pages/menu** directly should show a **Not Found** page. Confirm the Shop banner is still fully editable at **Website → Shop Banner** (the carousel). **Expect** the public **/blog** and **/menu** pages to look EXACTLY as before (byte-identical) — no wording moved, nothing on the storefront changed. **Red flag:** a blog field still shows in Site Content; the Blog wording or Menu entry is in the wrong place; /admin/pages/menu still loads an editor; or the public blog or shop page looks different.
- **T-299 (FAQ hero and Home banner copy/images get a real editor (orphan rescue 10 of 17)):** open **Website → Pages → FAQ** and **Expect** a **Page wording** card listing the FAQ hero **title** and **subtitle** as editable fields — they used to be ’orphans’ (rendered live on /faq but with NO editor anywhere). Then open **Website → Pages → Home** and **Expect** the Page wording card to list the eight ’Shop by Category’ and ’Shop by Brand’ banner fields — each band’s background image, eyebrow, title, and subtitle — all now editable in one place. Confirm the public **/faq** and **/** (home) pages look EXACTLY as before (byte-identical) until you deliberately edit and publish a field. **Expect** the **Site Content** screen to still be empty of these fields (they were never in the junk drawer — they had no home at all before this). **Red flag:** the FAQ or Home page editor has no Page wording card; a banner field is missing; or the public FAQ or home page looks different before you have edited anything.
- **T-300 (Home banner Page wording card removed (MIG-5a-fix: it was a duplicate of the Sections tab)):** open **Website → Pages → Home** and **Expect** the extra **Page wording** card that briefly appeared above the tabs (from MIG-5a) to be **GONE**. Confirm the two banners — **Shop by Category** and **Shop by Brand** — are still fully editable in the **Sections** tab (image, eyebrow, title, subtitle, button), exactly as they always were: that Sections tab is their real editor. The old home.category.*/home.brand.* text is only a behind-the-scenes fallback and is no longer shown as a second, confusing editor. Then open **Website → Pages → FAQ** and **Expect** its **Page wording** card (FAQ hero title + subtitle) to still be present — that one is a genuine field with no Sections banner behind it, so it correctly keeps its editor. **Expect** the public **/** (home) and **/faq** pages to look EXACTLY as before (byte-identical). **Red flag:** the Home editor still shows a Page wording card; the Sections banners changed; or the FAQ Page wording card disappeared.
- **T-301 (Last content orphans finished (MIG-5c: Specials hero text + Loyalty signup heading now editable; dead Menu hero text retired)):** open **Website → Specials** and **Expect** a new **Hero text** card at the top with three editable fields — the **eyebrow** (“Deals every day”), the **title** (“Cannabis Specials”), and the **subtitle** — each with a **Save draft**, **Preview**, and **Publish** button, and a snapshot history so you can roll back. Edit the title, Preview it, Publish it, and **Expect** the public **/specials** hero to update to your new text; before you edit, **Expect** the page to read EXACTLY as it does today (byte-identical). Next open **Website → Loyalty page** and **Expect** a new **Signup heading (above the form)** group at the TOP with two editable fields — the big “Signup to get offers and discounts from Greenway Marijuana” headline and its “Get updates on our promotions tailored to you.” subtitle; edit, Preview, Publish, and **Expect** the heading above the /loyalty signup form to follow — again byte-identical until you publish. Finally, confirm the **dead Menu hero text is gone for good**: there is NO editor anywhere for the old menu.hero.title/subtitle (the Shop banner carousel replaced that strip long ago), and the public **/menu** page looks exactly as today. **Red flag:** the Specials Hero text card or the Loyalty Signup heading group is missing; editing + publishing one of them does not change the matching public page; any of the three public pages (/specials, /loyalty, /menu) looks different BEFORE you make an edit; or a stray “Menu hero” text editor reappears.
- **T-302 (Site Content moves into Website Sync, part 1 (MIG-7 PR-A: new tabs on Website Sync; the Live preview and the SEO editor button relocated here)):** open **Website → Website Sync** and **Expect** it now shows **two tabs** across the top — **Storefront harmony** (the existing dashboard: published menu, deals this week, loyalty program, medical & hours — unchanged) and a new **Live preview** tab. Click **Live preview** and **Expect** the big storefront preview panel that used to live on the Site Content page: a row of page buttons (Homepage, Shop, Specials, About, Location, Price Match, Loyalty, Medical, Vendors, Blog, FAQ, and the three legal pages), a live iframe of the chosen page in Draft Mode, **Desktop / Tablet / Phone** size toggles, a **Refresh** button, and an **Open** (new-tab) link — pick a few pages and confirm each one renders exactly as it does on the live site. Back at the top of the page, **Expect** a **SEO editor →** button in the header that opens the existing SEO editor (**/admin/content/seo**) — the SEO editor page itself is unchanged and still works. Switching between the two tabs must keep the URL in step (the address gains **?tab=preview** on the preview tab) and never lose your place. This slice only ADDS these to Website Sync; the old Site Content page is retired in the next slice, so nothing is removed yet and nothing on the public site changes. **Red flag:** the two tabs do not appear; the Live preview tab is empty or the iframe/device toggles/Open link are missing; a previewed page looks different from the live site; the **SEO editor →** button is absent or lands on a broken page; or any figure on the Storefront harmony tab changed from before.

---
- **T-303 (Site Content moves into Website Sync, part 2 (MIG-7 PR-B: the old Site Content page is retired; its two live pieces already live on Website Sync)):** in the back office open the left sidebar and **Expect** the old **Site Content** link to be **gone** — everything it used to hold now lives on **Website Sync**. Go to **Website → Website Sync** and confirm its **two tabs** (**Storefront harmony** and **Live preview**) and the **SEO editor →** button are all still there and working exactly as they did after part 1. Now try to open the old page directly at **/admin/content**: **Expect** it to be **retired** (a **404 / not found**), NOT a broken half-page. Next check the editors that used to link back to Site Content: open **Settings → Branding**, **Header & Footer**, and **Legal & Policies** and **Expect** each one’s header button to now read **Website Sync →** and land on Website Sync (not a dead Site Content link). In any of those editors click **Save draft** / **Publish** on a block and **Expect** to be returned to the SAME editor you were on (not bounced to a 404) with your green **Saved.** / **Published.** banner. Open the **Carousel / Homepage** editor and use its **Preview** link: **Expect** it to open the **Website Sync Live preview** tab (the address gains **?tab=preview**), not the old Site Content page. Finally, walk a live public page in preview mode — **Expect** the small floating **pencil / edit** hotspots that used to sit on each block to be **gone for now** (they are switched off on purpose until that feature is rebuilt); the **Preview** badge and **Exit preview** button must still work. The public site itself must look **exactly as it does today** — nothing on any customer-facing page changes. **Red flag:** the Site Content link is still in the sidebar; /admin/content still opens a page; any Branding / Header & Footer / Legal editor still says ‘Site Content’ or lands on a dead link; saving/publishing from one of those editors throws a 404 instead of returning you to it; the Carousel Preview link opens the old page instead of the Website Sync Live preview tab; the edit pencils are still on a public preview; or any public page looks different from before.
- **T-304 (Home "Shop by Type" cards get owner-editable images + a dedicated "Type Cards" editor tab (PR 1: each of the six type tiles can now carry its own product photo, with the label in its own centered bottom bar; blank = a clean text-only card)):** on the public homepage scroll to the **Shop by Type** row and **Expect** the six tiles (**Flower, Pre-Rolls, Concentrates, Edibles, Liquids, Topicals**) to now wear the same **glow** theme as the product, vendor and specials cards, with each tile’s label sitting in its **own centered bottom bar** (not floating over the artwork). Because no owner images are set yet, **Expect** every tile to fall back to a **clean text-only card** — the site must look **exactly as it does today** for the customer. Click a tile and **Expect** it to still send you to the menu filtered to that type (the links are **unchanged**). Now go to the back office **Website → Homepage** editor and **Expect** a new **Type Cards** tab to sit alongside **Sections** (and Carousel / Q&A). Open it and **Expect** six labelled image pickers (one per type), each showing **no image (text-only)** and offering **Choose from Media Library**, a paste-URL box, and **Clear**. Pick an image for, say, **Flower**, click **Save**, and **Expect** a green **Category tile images published to the homepage.** banner and the badge to flip to **image set**. Reload the public homepage and **Expect** the Flower tile to now show that photo behind its centered bottom-bar label, while the other five stay text-only. Clear the Flower image, Save again, and **Expect** the tile to return to the clean text-only fallback. **Red flag:** the type tiles look different from the product/vendor/specials glow theme; the label overlaps the photo instead of living in its own bottom bar; a blank tile shows a broken image instead of clean text; a tile link no longer goes to the right filtered menu; the **Type Cards** tab is missing; saving throws a 404 instead of returning you to the Type Cards tab with the green banner; or a set image fails to appear on the live homepage.
- **T-305 (Home "Shop by Brand" now shows VENDOR cards (PR 2: the homepage brand row is grouped by producer -- same look as the Vendors page: name on top, logo in the middle, product count -- fixing the bug where only one card showed even though several vendors were on the menu; cards are static/unwired for now)):** on the public homepage scroll to the **Shop by Brand** row and **Expect** it to now show **vendor cards that look exactly like the Vendors page** — a dark **glow** tile with the **vendor name on top**, the **logo in the middle**, and the **product count** at the bottom, cycling the same green/gold/amber/teal glow as the product, type and specials cards. **Expect** to see **one card per producer** — the same vendors that appear on the **Vendors & Partners** page (open **/vendor-delivery** in another tab and confirm the names/logos/counts match). This fixes the old bug where the homepage showed only a **single brand card** even though the menu carried several vendors (the old grid grouped by the free-text *brand* field; it now groups by *vendor*, the same pipeline the Vendors page uses). A vendor with no uploaded logo yet **Expect** to fall back to the **placeholder logo** (never a broken image). For this slice the cards are **static / not clickable** on purpose — tapping one does **nothing** (a later slice wires them to the menu once the menu filter is switched from brand to vendor); the section header (**Featured Brands / Shop by Brand**) and the owner **card-count** control still work as before. The **Vendors & Partners** page itself must look and behave **exactly as it does today** (its cards still tap-to-reveal the blurb). **Red flag:** the homepage brand row still shows the old flat gradient brand tiles; it shows only one card when several vendors are stocked; the names/logos/counts disagree with the Vendors page; a card without a logo shows a broken image instead of the placeholder; tapping a homepage card navigates somewhere (it should be inert this slice); or anything on the Vendors & Partners page changed.
- **T-306 (Menu filter by VENDOR + Home vendor cards wired (PR 3: an additive Vendors filter sits directly above Brands on the shop menu and surfaces all brands by a producer; each Home Shop-by-Brand vendor card now links to that vendor-filtered menu; the Brands filter and every existing brand deep link are unchanged)):** open the public **/menu** and in the left filter sidebar **Expect** to see a new **Vendors** section sitting **directly above Brands** — same checkbox-row look, with its own **Search vendors** box. **Expect** each vendor row to show the **clean producer name** (e.g. *CERES*, not *CERES - 435011*) and a **live count**, matching the **Vendors & Partners** page and the homepage vendor cards. Tick a vendor and **Expect** the grid to narrow to **every product by that producer** — across all of that producer’s brands — with a removable **Vendor** pill appearing above the grid. Now go to the **homepage**, scroll to **Shop by Brand**, and **tap any vendor card**: **Expect** it to open **/menu** pre-filtered to that vendor (URL shows **?vendors=<name>**). Crucially, the **Brands** filter must still work exactly as before, and every existing brand link must still work: on a **product page** the **brand label still links** to its brand-filtered menu, and the **Thursday top-shelf** special / promotion **shop these brands** buttons still open their brand-filtered lanes. **Red flag:** there is no Vendors section, or it is below Brands instead of above; ticking a vendor changes nothing or filters wrong; the vendor names show trailing license numbers or disagree with the Vendors page; a homepage vendor card does nothing or points at the wrong vendor; the Brands filter or any brand deep link (product-page brand link, Thursday special, promotion brand button) stopped working.
- **T-307 (Partner-With-Greenway desktop polish (PR 4: on the Vendors & Partners page the outreach intro paragraph now fills the container width on desktop, the vendor-contact email cards are short and wide with no vertical dead space, and the on-card Parsed automatically footnote is removed; mobile is unchanged)):** on a **desktop** browser open the public **Vendors & Partners** page and scroll to the **Partner With Greenway** panel. **Expect** the intro paragraph under the heading to **span the full width** of the panel — not a narrow centered column. **Expect** the five contact cards (Sample Drops, Promotions & Deals, Vendor Days, Send Us Your Menu, Transfer Manifests) to be **short and wide**, spread across the full width, each only as tall as its own text — **no large empty gap** between the blurb and the email address. **Expect** the **Send Us Your Menu** and **Transfer Manifests** cards to **no longer show the Parsed automatically footnote**. Now shrink to a **phone width** (or open on a phone): **Expect** the section to look exactly as it did before this change (cards stacked one per row, intro paragraph centered), also with **no** Parsed automatically footnote. **Red flag:** on desktop the intro paragraph is still a narrow centered column; the cards are tall with a big empty gap above the email; the Parsed automatically footnote still shows anywhere; or the mobile layout changed.
- **T-308 (Partner-With-Greenway equal-height cards (PR 5: the five vendor-contact email cards in the Partner With Greenway panel are now all the same height on desktop; short cards may show empty space above the email, which is the intended trade-off; mobile is unchanged)):** on a **desktop** browser open the public **Vendors & Partners** page and scroll to the **Partner With Greenway** panel. **Expect** all five contact cards (Sample Drops, Promotions & Deals, Vendor Days, Send Us Your Menu, Transfer Manifests) to be **exactly the same height** — their top edges line up and their bottom edges line up in one clean row. **Expect** the email address at the bottom of each card to sit on the **same baseline** across all five. Shorter cards may show some **empty space above the email** and that is intentional. Now shrink to a **phone width**: **Expect** the section to look exactly as it did before (cards stacked one per row). **Red flag:** the cards are different heights (ragged bottom edge); the email addresses sit at different vertical positions; or the mobile layout changed.
- **T-309 (Price Match page fully owner-editable (PR: every text element on the Price Match page is now a controlled content block listed in page order in the page editor, plus a real editable image slot with the Canva helper and a Generate-with-FLUX link replaces the placeholder artwork; defaults are byte-identical so the public page is unchanged until a block is edited and published)):** as an admin open **Admin > Pages > Price Match**. **Expect** a **Page wording** card listing the editable spots in the SAME top-to-bottom order as the page — hero title, the "Guaranteed savings" eyebrow, the "Our Price Match Promise" headline, the intro paragraph, the graphic, the "Details:" heading, then the 8 rules. Edit the hero title to a test word, **Publish**, open the public **Price Match** page and **Expect** the new word; then restore it. On the graphic slot **Expect** a **Canva** sizing hint and a **Generate with FLUX (Creative Studio)** link, plus **Choose from Media Library**; set an image, Publish, and **Expect** it to fill the card panel; clear it and **Expect** the built-in placeholder artwork to return. **Red flag:** the editor lists spots out of page order; the public page changed before any block was edited; the intro apostrophe or any rule text differs from the original; or the FLUX/Canva helpers are missing from the image slot.
- **T-310 (About page fully owner-editable, incl. reorderable Core Values (PR: every text element on the About page is now a controlled content block listed in page order in the page editor, plus the Our Values cards get their own owner editor to add, edit, delete, and reorder them while keeping their exact look, backed by migration 0149 about_core_values with a byte-identical pre-migration fallback; defaults are byte-identical so the public page is unchanged until a block is edited and published)):** as an admin open **Admin > Pages > About**. **Expect** a **Page wording** card listing the editable spots in the SAME top-to-bottom order as the page — the hero title and subtitle, the orange intro heading ("Your Most Trusted Cannabis Dispensary"), the two intro paragraphs, then the "Our Values" heading. Edit the intro heading to a test word, **Publish**, open the public **About** page and **Expect** the new word; then restore it. Below the wording card **Expect** a **Core values cards** editor showing your four values (01-04); change a title, use the up/down arrows to reorder, click **+ Add value** to append a fifth, **Auto-number 01, 02, 03**, **Save draft** then **Publish**, and **Expect** the About page's Our Values grid to match (same look, same styling). Delete the added card and republish to restore. **Red flag:** the editor lists spots out of page order; the public page changed before anything was edited; the intro apostrophe or any value's text differs from the original; the values grid loses its centered orange-number / white-title / grey-summary look; or the section blanks out before a first publish.
- **T-311 (Locations page fully owner-editable (PR: every text element on the Locations page is now a controlled content block listed in page order in the page editor -- hero title, city line, hours pill, the About heading, write-up and button label, the Store Details heading and all four detail values (address, phone, hours, email), plus the map eyebrow, heading and button label; the actionable links (directions, tel:, mailto:, /menu) stay derived from the shared business record so editing a label never breaks a link or changes the dialed number, and the map iframe stays non-editable; no migration is needed and defaults are byte-identical so the public page is unchanged until a block is edited and published)):** as an admin open **Admin > Pages > Locations**. **Expect** a **Page wording** card listing the editable spots in the SAME top-to-bottom order as the page — the hero title, the city line ("Port Orchard, WA 98367"), the hours pill ("open Until 11:00 PM"), the About heading, the About write-up, the "SHOP OUR MENU" button label, the "Store Details" heading, the address, phone, hours and email values, then the map eyebrow ("Find Us"), the map heading ("Map & Directions"), and the "Get Directions" button label. Edit the store hours to a test value, **Publish**, open the public **Locations** page and **Expect** the new hours in the Store Details card; then restore it. Confirm the phone label is editable but the **tel:** link still dials the real number, the email label drives a matching **mailto:** when it is a valid address, and the map iframe below is unchanged. **Red flag:** the editor lists spots out of page order; the public page changed before anything was edited; the storefront photo, the About write-up wording, or any detail value differs from the original; editing the phone or email label breaks the tel:/mailto: link or changes the dialed number; or the map iframe becomes editable or disappears.
- **T-312 (Blog / newsletter cards get our signature glow in their type color (PR: each blog + newsletter card now has the same edge-lit glow as the product / vendor / specials cards, colored to MATCH its category pill -- PRODUCTS green, DEALS orange, CULTURE our indica blue with the white pill unchanged, NEWSLETTER gold -- and the glow brightens on hover on top of the existing slight zoom and lift; the glow reuses the one shared recipe in glow-card-core via a new pinned per-category tone map, so it stays consistent with every other glowing card on the site)):** open the public **Blog** page (or the newsletters view). **Expect** every card to show our signature edge-lit glow — and the glow color to MATCH that card's colored type pill: green for **Products**, orange for **Deals**, our indica **blue** for **Culture** (the pill itself stays white), and yellow/gold for **Newsletters**. Hover a card and **Expect** the glow to get noticeably brighter while the card does its usual slight zoom-in and lift. Confirm the four colors read as the SAME glow family as the product, vendor, and specials cards elsewhere on the site. **Red flag:** any card has no glow, a glow color that doesn't match its pill, a Culture pill that turned blue (it should stay white), a glow that doesn't brighten on hover, or a card that lost its zoom / lift.
- **T-313 (One consistent textured background across every customer page (PR: the storefront pages that used to have a plain-black background -- Home, Shop, Locations and Vendors -- now show the SAME textured brand gradient the other pages already had (a soft orange, gold and greenway-green glow over black, with a fine noise grain), so the background never jumps between plain black and textured as you move around the site; the gradient is the About-page recipe -- the one page that already used all three brand colors -- pinned in one shared core and painted once per page by a new shared SiteBackground layer; no content or data changed and every card / photo sits on top exactly as before)):** visit the storefront and move through **Home**, **Shop**, **Locations**, **Vendors**, **About**, **FAQ**, **Blog** and **Specials**. **Expect** the SAME textured background on every page — a soft orange glow toward the top-left, gold toward the top-right and a subtle greenway-green glow low-center, over black, with a faint noise grain. **Expect** the four pages that used to be flat black (Home, Shop, Locations, Vendors) to now match the others, with no page snapping back to plain black as you navigate. Confirm every product card, vendor tile, banner and photo still sits cleanly on top and nothing is covered or clickable-through. **Red flag:** any customer page still shows a plain / very dark black background; the gradient colors or positions differ between pages; a card or banner looks washed out or the texture bleeds over content; or the background intercepts clicks.
- **T-314 (Manual GPT-4o AI product/strain lookup on Product Onboarding (PR: each draft row now has a Google-style AI Lookup box and Search button that the operator presses on demand -- it never runs on its own; GPT-4o searches the live internet for the strain or product and, when it is at least 90% confident about the strain type, fills the row's strain-type field for you, and otherwise says plainly it found nothing reliable rather than guessing; when it turns up more detail it offers a Save-to-KB draft for the owner to approve so future lots auto-attach it; it works for all products, not just flower, and it obeys the compliance line -- experiential vibe words like relaxing, energizing, sleepy and appetite are allowed, but medical/curative claims such as 'reduces inflammation' are blocked on screen and before any save)):** open **Product Onboarding** (Inventory — Product Onboarding) with a few draft rows. On a row, click **AI Lookup**, confirm the box is prefilled with the product name and brand, then press **Search**. **Expect** GPT-4o to look it up and, when it is — at least 90% confident — about the strain type, autofill the row's strain-type picker (marked "AI-filled, review") with the source links it used shown below. **Expect** an honest "couldn't find anything reliable" message (never a guess) when it isn't confident. When it finds more, **expect** a **Save to KB?** button that saves a DRAFT for you to approve (nothing publishes on its own). Confirm experiential words (relaxing, energizing, sleepy, appetite) are kept but a medical claim like "reduces inflammation" or "cures ..." is dropped. **Red flag:** the lookup runs without pressing the button; a low-confidence result autofills the field; a medical/curative claim appears or gets saved; the Save-to-KB action publishes instead of drafting; or the box isn't prefilled with the row's name and brand.
- **T-315 (All-inclusive AI Lookup that feeds enrichment (PR: the operator-triggered AI Lookup on Product Onboarding now works for every product type, not just flower, and one search gathers everything -- strain type plus a description, short description, category, cannabinoid ratio, size, and reviewable image candidates; on Save-to-KB it stages strain facts as a kb_strains draft and, for any product with a POS key, stages the description and image candidates as pending drafts on the enrichment page so the assets are waiting there with no second search; everything stays drafts-only with human approval -- nothing publishes and no image imports on its own -- and the compliance line is unchanged: experiential vibe words allowed, medical/curative claims blocked)):** open **Product Onboarding** (Inventory — Product Onboarding) and, on a NON-flower row (e.g. an edible or beverage like "Ray's Lemonade Raspberry 1:1 12oz"), click **AI Lookup** and press **Search**. **Expect** the result to show a description, and where verifiable a category, ratio (e.g. 1:1), size, and small image-candidate thumbnails labeled "review on the enrichment page" — with the strain type left alone (no autofill for non-flower). Press **Save to KB?**; **expect** a confirmation that details and images were staged for enrichment. Open that product on the **enrichment** page and **expect** the description and image candidates waiting there as PENDING drafts to Accept or Import — nothing applied automatically. **Red flag:** an image imports or a description publishes without your approval; a medical/curative claim appears or is staged; the lookup runs without pressing the button; or a draft with no POS key silently loses its details instead of telling you it can't stage them yet.
