...
- Companion documents this checklist leans on (all in the repo):
  ...
  - `docs/MIGRATIONS_TO_RUN.md` — the manual migration checklist (files 0054 → 0135).
...
- [ ] **C-040 👤 Migration sweep:** open `docs/MIGRATIONS_TO_RUN.md` and confirm
  every box through **0135** (the current last file; 0135 = store safe (twice-daily manager counts + register change swaps, $1,000 target), 0134 = tips-at-close on drawer sessions (employee money, kept out of over/short math), 0133 = special discount programs (employee/industry/veteran settings + use tracking), 0132 = GW-027 rejected-rows visibility columns, 0131 = GW-009 Pacific-day medical ledger default, 0130 = GW-019/GW-020 RLS + insider-threat hardening, 0129 = GW-011/GW-012 concurrency guards, 0128 = GW-023 stranded-sale recovery, 0127 = GW-018 inactive-by-default staff profiles) is checked as run. Special
...
- [ ] **C-110 ⛔ FINAL GATE — read each aloud and check it:**
  ...
  - Stage 3: migrations swept through 0135; Supabase auth/backups/PITR set;
...
| Migrations manual, 135 files, only 0061 re-run needed; 0123/0120–0122 critical | `docs/audit/OWNER-TASKLIST.md` §1; `docs/MIGRATIONS_TO_RUN.md`; `supabase/migrations/` (last file 0135) |
