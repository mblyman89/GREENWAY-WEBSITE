 ...
 *
 * COVERAGE GATES LIVE IN `timesheet-mentor-gates.ts`, NOT HERE. Standing rule
 * 26 still applies - every engine ships a mentor layer, and the gate reads the
 * core module FROM DISK rather than trusting that someone remembered to update
 * a list. The gates simply cannot live in THIS file, and books-33 found out the
 * hard way why.
 *
 * THE DEFECT THIS SPLIT REPAIRS. TimesheetWorkbench.tsx is a `"use client"`
 * component and it imports the lessons below. That makes this module part of a
 * BROWSER bundle. The gates called `readFileSync`, so `node:fs` was dragged
 * into the browser graph, and Turbopack refused:
 *
 *     the chunking context (unknown) does not support external modules
 *     (request: node:fs)
 *
 * Every Vercel build - preview and production - failed on that, while GitHub
 * Actions stayed green, because CI runs vitest and the migrations and never
 * once runs `next build`. Michael could not see any new work for several
 * slices. The lesson is recorded as standing rule 65.
 *
 * So: LESSON DATA lives here and is pure and browser-safe. DISK-READING GATES
 * live in the sibling `-gates.ts`, are imported only by tests, and are just as
 * strict as they were before. Nothing was weakened to make the build pass.
 */
