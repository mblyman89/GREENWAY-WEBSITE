/**
 * src/lib/weedmaps/ai.ts
 *
 * AI DRAFT assist for WeedMaps menu product descriptions (Slice 41).
 *
 ...
 * or pushed to WeedMaps. The draft is returned as plain text and re-checked for
 * compliance language.
 *
 * Grounded in the existing AI infrastructure (generateStructured + schema + compliance)
 * and the WeedMaps data rules (see docs/weedmaps-menu-api.md). Never invents cannabinoid
 * figures, brands, or strains — it only describes what it is given.
 *
 * Server-only.
 */
import "server-only";
...
export type WeedmapsDescriptionDraft = {
  description: string;
};

const descriptionSchema = defineSchema<WeedmapsDescriptionDraft>("weedmaps_description", {
  description: {
    kind: "string",
    description:
      "A plain-text product description for a WeedMaps menu, 1-3 sentences, no markup, no medical/therapeutic claims.",
    minLength: 20,
    maxLength: 600,
  },
});
...
/**
 * Draft a single plain-text WeedMaps description. Throws AiNotConfiguredError when AI is
 ...
 */
export async function draftWeedmapsDescription(
  ...
  const system = [
    ...
    "cannabis retailer's WeedMaps menu. Output PLAIN TEXT only — no HTML, markdown, lists,",
  ...
  const draft = await generateStructured<WeedmapsDescriptionDraft>({
    system,
    user,
    ...
    temperature: 0.4,
    maxTokens: 300,
    context: { feature: "weedmaps.description", entityType: "menu_item" },
  });
