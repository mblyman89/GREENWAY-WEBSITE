...
export function AiComplianceFlags({
  ...
  if (flags.length === 0) {
    ...
    return (
      <p className="inline-flex items-center gap-1.5 text-[11px] text-[var(--admin-accent)]">
        <span>✓</span> No compliance concerns detected
      </p>
    );
  }
  return (
    <div className="rounded-lg border border-[var(--admin-gold)]/30 bg-[var(--admin-gold)]/[0.06] px-3 py-2">
      <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--admin-gold)]">
        ⚠ Review before publishing — {flags.length} flag{flags.length === 1 ? "" : "s"}
      </p>
      <ul className="mt-1 space-y-0.5">
        {flags.map((f, i) => (
          <li key={i} className="text-[11px] text-[var(--admin-gold)]/90">
            • {f}
          </li>
        ))}
      </ul>
