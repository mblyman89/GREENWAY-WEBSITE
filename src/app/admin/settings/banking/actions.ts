...
import { saveAchCompanySettings, type AchCompanySettings } from "@/lib/payroll/payroll-store";
...
export async function saveBankingSettingsAction(formData: FormData): Promise<void> {
  ...
  const input: AchCompanySettings = {
    ...
    company_account_number: String(formData.get("company_account_number") ?? "").trim(),
