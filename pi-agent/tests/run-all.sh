...
banner "1/4  Agent self-test (the ~78 checks that ship on the Pi)"
python3 "$HERE/../greenway_announcer.py" selftest || FAILED=1

banner "2/4  End-to-end against a real HTTP server"
python3 "$HERE/test_e2e.py" || FAILED=1

banner "3/4  Testing the tests: mutating the pure logic"
bash "$HERE/mutation-selftest.sh" | tail -4
bash "$HERE/mutation-selftest.sh" | grep -q "MUTATION ROUND CLEAN" || FAILED=1

banner "4/4  Testing the tests: mutating end-to-end behaviour"
