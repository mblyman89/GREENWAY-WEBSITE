...
--
-- NOTE: every DELETE below carries an explicit `where true`. This is intentional
-- and required: Supabase/Postgres can run with a safeguard that rejects an
-- unqualified DELETE with "DELETE requires a WHERE clause". `where true` matches
-- ALL rows (so the wipe behavior is unchanged) while satisfying that guard, and
-- keeps the function safe to re-apply.
-- =============================================================================
