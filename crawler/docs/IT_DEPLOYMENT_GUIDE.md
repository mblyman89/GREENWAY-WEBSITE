...
| Crawl finishes instantly with `1 ok · 0 drafts` on an **age-gated** cannabis site | Fixed in C7. (Old cause: the C3 overlay remover stripped the "Are you 21?" modal *and* the real content behind it → thin shell → no links, no text.) | Update the worker to the C7 build (`git pull` + restart). Overlay removal is now OFF by default and the crawler clicks "YES" on the age gate. **Then clear the 24 h page cache once** so the stale shell isn't re-served: systemd → `rm -f crawler/.cache/page_*.json`; Docker → `docker exec <container> sh -c 'rm -f /tmp/**/page_*.json'` (or just restart with a fresh cache dir). Alternatively re-run the harvest with `force_fresh` to bypass the cache for that job. |
